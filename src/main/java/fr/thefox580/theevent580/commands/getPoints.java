...
public class getPoints implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        if (player != null){
            if (player.isOnline()){
                long pointsPlayer = this.plugin.getConfig().getInt("points."+player.getUniqueId());

                if (commandSender instanceof Player){
                    Player sender = Bukkit.getPlayer(commandSender.getName());
                    String strPointsPlayer = String.valueOf(pointsPlayer);
                    ...
                    String message = Component.text("Player ")
                            ...
                            .append(Component.text(" currently has " + strPointsPlayer + " points", TextColor.color(255, 255, 255))))).toBuilder().build().content(); //Set custom message
                    ...
                    commandSender.sendMessage("Player " + player.getName() + " currently has " + pointsPlayer + " points");
                }
            }
        }
